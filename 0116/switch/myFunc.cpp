...
class Neuron{
	...
	public:
		...
		int** convolution(int **M, double**F);
...
void Neuron::setPS(int pad, int strr, int strc, int Mr, int Mc, int F){
	...
	double row=(this->M_sizer-this->F_size+2*this->padding)/this->strider+1;
	double col=(this->M_sizec-this->F_size+2*this->padding)/this->stridec+1;
	...
	// Stirde value check
	if((this->M_sizec-this->F_size+2*this->padding)%this->stridec!=0){printf("\nStride size ERROR!!!\n"); abort();}
	if((this->M_sizer-this->F_size+2*this->padding)%this->strider!=0){printf("\nStride size ERROR!!!\n"); abort();}
}
...
int** Neuron::convolution(int **M, double **F){
