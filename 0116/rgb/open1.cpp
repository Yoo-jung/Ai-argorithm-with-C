...
int main(){
	...
	for (int t=0;t<3;t++){
		...
		*(Result+t)=(int**)malloc(sizeof(int*)*M_size_r);
		...
		// color
		for (int m=0;m<M_size_r;m++){
			for (int mm=0;mm<M_size_c;mm++) {Result[t][m][mm]=M[m][mm];}
		}
		int bufr = M_size_r/2;
		int bufc = M_size_c/2;
		if(t==0){
			for (int m=0;m<bufr;m++){
				for (int mm=0;mm<bufc;mm++) {Result[t][m][mm]=M[m][mm]*0;}
			}
		}else if(t==1){
			for (int m=bufr;m<2*bufr;m++){
				for (int mm=0;mm<bufc;mm++) {Result[t][m][mm]=M[m][mm]*0;}
			}
		}
		else if(t==2){
			for (int m=0;m<bufr;m++){
				for (int mm=bufc;mm<2*bufc;mm++) {Result[t][m][mm]=M[m][mm]*0;}
			}
		}
	...
	imwrite("outme_a.bmp",img_output);
	return 0;
}
